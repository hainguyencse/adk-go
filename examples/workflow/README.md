...
Runnable samples for the adk-go **graph workflow engine** (`google.golang.org/adk/workflow` + `agent/workflowagent`). Each sample is a `main` package that builds a `workflowagent` from a graph of nodes and serves it through the console launcher.
